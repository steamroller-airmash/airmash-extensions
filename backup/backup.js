...
(function() {
    ...
    SWAM.on("gameRunning", function() {
        const CLIENTPACKET = Network.CLIENTPACKET;
        const SERVERPACKET = Network.SERVERPACKET;
    ...
    SWAM.on("gamePrep", function() {
        var url = "wss://game-" + game.playHost + ".airma.sh/" + game.playPath;
        conn = new WebSocket(url);
        conn.binaryType = "arraybuffer";

        conn.onopen = function() {
            conn.send(encodeMessage({
                c: Network.CLIENTPACKET.BACKUP,
                token: game.myToken
            }));
        };
        conn.onmessage = function(msg) {
            console.log(decodeMessage(msg.data));
        };
    });
    ...
    SWAM.registerExtension({
        name: "Backup",
        id: "steamroller-backup",
        description: "Open another backup connection.",
        version: "0.0.1",
        author: "STEAMROLLER"
    });
})();
